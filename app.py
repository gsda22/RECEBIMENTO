...
import datetime
import pytz
import json # NOVO: Importa biblioteca para ler o JSON

# --- CONFIGURAÇÕES DA PLANILHA (CONFIRME) ---
PLANILHA_ID = "1pvPr2wDSnPpO4Vi0vE4kHwVCk79YIMs1XtoZ8TYmAsg"
NOME_ABA = "Página1"
CREDENTIALS_FILE = "gcp_credentials.json" # NOVO: Nome do arquivo de credenciais
...
def get_sheets_client():
    """Conecta ao Google Sheets lendo o arquivo JSON de credenciais diretamente."""
    
    try:
        # Tenta ler as credenciais do arquivo JSON
        ...
        st.info("Crie o arquivo gcp_credentials.json na mesma pasta do app.py e cole o JSON completo.")
        st.stop()
    except json.JSONDecodeError:
        st.error(f"ERRO: O arquivo '{CREDENTIALS_FILE}' não é um JSON válido. Verifique se o conteúdo está correto.")
        st.stop()
        
    try:
        # Passa o dicionário JSON nativo para o gspread (solução para o erro anterior)
        gc = gspread.service_account_from_dict(creds_dict)
        return gc
    except Exception as e:
        st.error(f"Falha na Autenticação (gspread). Verifique se o e-mail '{creds_dict.get('client_email', 'N/D')}' está como Editor na sua planilha. Erro: {e}")
        st.stop()
...
# ----------------------------------------------------
# 2. FLUXO PRINCIPAL DO STREAMLIT
# ----------------------------------------------------
        ...
        if submitted_fechamento:
            if encarregado and auditor:
                ...
                try:
                    ...
                    st.error(f"Falha ao registrar na Planilha. Verifique as permissões de Editor. Erro: {e}")
            else:
...
# --- FASE 4: CONCLUSÃO E REINÍCIO ---
    ...
    # Botão para iniciar novo lançamento
    if st.button("Iniciar Novo Lançamento"):
        # Limpa todos os dados de sessão para recomeçar
